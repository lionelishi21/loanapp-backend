...
class LoanApplicationApprovedSms extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toATSms($notifiable)
    {
        ...
        $template = SmsTemplate::where('name', 'loan_application_approved')->get()->first();
        $body = $template['body'];
        ...
        $body = str_replace('{loan_reference_number}',  $notifiable['loan_reference_number'], $body);
        $body = str_replace('{repayment_period}',  $notifiable['repayment_period'], $body);
        $body = str_replace('{start_date}',  $notifiable['start_date'], $body);
